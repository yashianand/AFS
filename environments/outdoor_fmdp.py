...
class OutdoorMDP:
    def __init__(self, grid, is_oracle=False, is_lrtdp=True, calibration_reward=[]):
        ...
        self.domain = 'outdoor'
        self.start_state = (((np.where(self.grid == b'A')[0].item(), np.where(self.grid == b'A')[1].item()), True, False))
        self.reset()
    ...
    def reset(self, full_grid=True):
        ...
        self.terminal_state = (((np.where(self.grid == b'G')[0].item(), np.where(self.grid == b'G')[1].item()), True, False))
        self.state = (((np.where(self.grid == b'A')[0].item(), np.where(self.grid == b'A')[1].item()), True, False))
    ...
    def getStateFactorRep(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if currState == 'G':
                    self.terminal_state = ((i, j), True, False)
    ...
    def move(self, currFactoredState, action):
        state, grass, puddle = currFactoredState
        ...
        if self.is_boundary(new_state):
            ...
            if self.grid_list[new_state[0]][new_state[1]] == '$':
                return (new_state, False, False), False
            elif self.grid_list[new_state[0]][new_state[1]] == 'P':
                return (new_state, False, True), False
            else:
        ...
        if self.is_baseline==True:
            ...
            if action in [0,1,2,3]:
                if is_oracle or self.get_all_reward:
                    if self.is_goal(state) == True:
                        return goal
                    elif road==False and puddle==True:
                        return severe_nse
                    elif road==False and puddle==False:
                        return mild_nse
                    else:
                        return step
                else:
